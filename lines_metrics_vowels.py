...
from string_pars_functions import line_position, accent_vowels, ikt_schema, pos_stream, negation, ili, vowels_count
import codecs, re
...
def main():
    t = codecs.open('metrics_table_no_context_vowels.csv', 'w', 'utf-8')
    t.write('anomalia\tposition\twords_count\taccent_vowels\ttf_idf\tpos\tnames\tpunctuation_in_the_middle\tquestion\tnegation\tline_sentence\tnumerals\tsubject\tverbs\tadjectives\tadverbs\tpersonal_pronomen\tprepositions\tpart\tconjunction\tconj_constructions\to\tu\ta\te\ti\ty\n')
    ...
    for line in text:
        ...
        o, u, a, e, i, y = vowels_count(acc_v)
        ...
        constr = str(ili(line))
        primary_line_metrics[line_id] = [ano, position, words_count, acc_v, tf_idf, pos, names, punct, quest, neg, sent, str(num), str(s), str(v), str(a), str(adv), str(spro), str(pr), str(part), str(conj), constr, str(o), str(u), str(a), str(e), str(i), str(y), str(ikt)]
        table_line = u'\t'.join(primary_line_metrics[line_id][:-1])
        t.write(table_line + '\n')
    t.close()
    for window in nwindow:
        
        t = codecs.open('metrics_table_with_context_' + str(window) + '_vowels.csv', 'w', 'utf-8')
        t.write('anomalia\tposition\twords_count\taccent_vowels\ttf_idf\tpos\tnames\tpunctuation_in_the_middle\tquestion\tnegation\tline_sentence\tnumerals\tsubject\tverbs\tadjectives\tadverbs\tpersonal_pronomen\tprepositions\tpart\tconjunction\tconj_constructions\to\tu\ta\te\ti\ty\tprev_word_count\tprev_accent_vowels\tprev_tf_idf\tprev_names\tprev_punctuantion_in_the_middle\tprev_question\tprev_negation\tprev_line_sentence\tprev_numerals\tprev_subject\tprev_verbs\tprev_adjectives\tprev_adverbs\tprev_personal_pronomen\tprev_prepositions\tprev_part\tprev_conjunction\tprev_conj_constructions\tprev_ikt_schema\tprev_o\tprev_u\tprev_a\tprev_e\tprev_i\tprev_y\tpost_word_count\tpost_accent_vowels\tpost_tf_idf\tpost_names\tpost_punctuantion_in_the_middle\tpost_question\tpost_negation\tpost_line_sentence\tpost_numerals\tpost_subject\tpost_verbs\tpost_adjectives\tpost_adverbs\tpost_personal_pronomen\tpost_prepositions\tpost_part\tpost_conjunction\tpost_conj_constructions\tpost_ikt_schema\tpost_o\tpost_u\tpost_a\tpost_e\tpost_i\tpost_y\n')
        for line_id in primary_line_metrics:
            arr_metrics = primary_line_metrics[line_id]
            arr_metrics1 = arr_metrics[:-1]
            arr_metrics1 = context(arr_metrics, arr_metrics1, 'prev', line_id, primary_line_metrics, window)
            arr_metrics1 = context(arr_metrics, arr_metrics1, 'forw', line_id, primary_line_metrics, window)
            table_line = u'\t'.join(arr_metrics1)
            t.write(table_line + '\n')
        t.close()

def context(arr_metrics, arr_metrics1, direction, line_id, primary_line_metrics, window):
    prev_word_count = 0
    prev_acc_v = u''
    prev_tf_idf = 0
    prev_names = 0
    ...
    prev_array = [str(prev_word_count), prev_acc_v, str(prev_tf_idf), str(prev_names), str(prev_punkt), str(prev_quest), str(prev_neg), str(prev_sent), str(prev_num), str(prev_s), str(prev_v), str(prev_a), str(prev_adv), str(prev_spro), str(prev_pr), str(prev_part), str(prev_conj), str(prev_constr), str(prev_ikt), str(o), str(u), str(a), str(e), str(i), str(y)]
